...
#' algo_leverage()
#' 
#' @return a list contains two numeric vectors, the simulated design matrix \eqn{X} and the simulated response matrix \eqn{Y}. 
#' The two numeric vectors corresponding to uniform sampling (beta_unif) and leverage sampling (beta_lev). 
#' Each vector contains regression coefficients obtained using r = 10, 50, 100, 200, 300 in order.
#' 
#' @export
...
algo_leverage=function(){
  ...
  return(list(beta_unif=beta_unif,beta_lev=beta_lev,X=X,Y=Y))
}